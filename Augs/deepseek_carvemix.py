...
from tensorflow.keras.mixed_precision import experimental as mixed_precision  # Import mixed precision API
...
def load_and_preprocess(file_path, is_mask=False):
    if not os.path.exists(file_path):
        ...
        return np.zeros((IMG_SIZE, IMG_SIZE))  # Return an empty array if the file doesn't exist
    img = cv2.imread(file_path, cv2.IMREAD_GRAYSCALE)
    if img is None:
        print(f"Warning: Failed to load image: {file_path}")
        return np.zeros((IMG_SIZE, IMG_SIZE))  # Return an empty array if the image fails to load
...
def get_distance(mask, spacing=(1,1)):
    """Return signed distance map."""
    dist_func = ndimage.distance_transform_edt
    distance = np.where(mask, -(dist_func(mask, sampling=spacing)),
                       dist_func(1-mask, sampling=spacing))
    return distance

def carvemix(img_a, img_b, mask_a, mask_b):
    """Implement CarveMix augmentation."""
    dist_array = get_distance(mask_a)
    c = np.random.beta(1, 1)  # [0,1]
    c = (c-0.5)*2  # [-1,1]
    if c > 0:
        lam = c * np.min(dist_array)/2
    else:
        lam = c * np.min(dist_array)
    mixing_mask = (dist_array < lam).astype('float32')
    new_img = img_a * (1-mixing_mask) + img_b * mixing_mask
    new_mask = mask_a * (1-mixing_mask) + mask_b * mixing_mask
        ...
        for case_id in batch_ids:
            ...
            for f in input_files:
                ...
                if img.size == 0 or mask.size == 0:
                    continue
                # Apply CarveMix with 50% probability
                if np.random.rand() < 0.5:
                    # Randomly select another image/mask
                    other_id = np.random.choice(self.list_IDs)
                    other_files = sorted([f for f in os.listdir(INPUT_PATH)
                                       if f.startswith(f'slice_{other_id}') and f.endswith('.png')])
                    if other_files:
                        other_f = np.random.choice(other_files)
                        other_img = load_and_preprocess(os.path.join(INPUT_PATH, other_f))
                        other_mask = load_and_preprocess(os.path.join(MASK_PATH, other_f), is_mask=True)
                        img, mask = carvemix(img, other_img, mask, other_mask)
                X.append(img)
                y.append(mask)
        if not X or not y:
            return np.zeros((0, IMG_SIZE, IMG_SIZE, 1)), np.zeros((0, IMG_SIZE, IMG_SIZE, 1))
        return np.expand_dims(np.array(X), -1), np.expand_dims(np.array(y), -1)

def create_model(inputs = Input((IMG_SIZE, IMG_SIZE, 1))):
    # Encoder
