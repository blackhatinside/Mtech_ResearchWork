...
# HIMRA: Advanced Physiological Augmentation for SISS2015 Dataset
'''
HIMRA: Hemodynamically Informed Mixed Reality Augmentation adapted for SISS2015 dataset
This implementation specifically targets DWI modality with corresponding masks from the SISS2015 corpus.
'''
...
# DWI modality = 1, Mask modality = 5 (based on previous discussion)
DWI_MODALITY = 1
MASK_MODALITY = 5
...
def visualize_himra_augmentation(sample_id, slice_num, output_dir):
    """
    ...
    image_path = os.path.join(sample_dir, dwi_filename)
    mask_path = os.path.join(sample_dir, mask_filename)
    ...
    plt.suptitle(f"HIMRA Augmentation Steps - Sample {sample_id}, Slice {slice_num}", fontsize=16, y=1.05)
    ...
    filename = f'sample_{sample_id}_slice_{slice_num}_himra_steps.png'
...
if __name__ == "__main__":
    ...
    sample_ids = list(range(1, 29))  # 28 samples total
    ...
    # Visualize HIMRA augmentation on sample images
    output_dir = os.path.join(OUTPUT_DIRECTORY, 'himra_visualizations')
    os.makedirs(output_dir, exist_ok=True)

    # Test samples to visualize - using SISS2015 samples
    test_samples = [
        (1, 66),  # Sample 1, slice 66
        (14, 66), # Sample 14, slice 66
        (28, 75), # Sample 28, slice 75
    ]

    # Generate and save visualizations
    for sample_id, slice_num in test_samples:
        save_path = visualize_himra_augmentation(sample_id, slice_num, output_dir)
        print(f"Saved HIMRA visualization for sample {sample_id}, slice {slice_num} to {save_path}")
