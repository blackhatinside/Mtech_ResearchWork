...
def get_original_file_pairs():
    valid_pairs = []
    case_ids = get_case_ids(INPUT_PATH)
    for case_id in case_ids:
        input_dir = INPUT_PATH
        mask_dir = MASK_PATH
        input_files = sorted([f for f in os.listdir(input_dir) if f.startswith(f'slice_{case_id}') and f.endswith('.png')])
        for f in input_files:
            mask_path = os.path.join(mask_dir, f)
            if has_lesion(mask_path):
                valid_pairs.append({
