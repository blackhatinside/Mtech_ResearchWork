...
def simulate_hemodynamics(image, mask, lesion_class):
    contrasts = {1: (0.3, 1.7), 2: (0.4, 1.6), 3: (0.5, 1.5), 4: (0.6, 1.4), 5: (0.7, 1.3)}
...
def attention_occlusion(image, mask):
    # Create brain mask (non-zero pixels in image)
    brain_mask = (image != 0)
    ...
    if np.sum(mask) > 0:
        lesion_area = mask > 0
        attention_field[lesion_area] *= 0.5
        dilated = ndimage.binary_dilation(mask.astype(bool), iterations=3)
        boundary = dilated & (mask == 0)
        attention_field[boundary] = np.random.uniform(0.6, 1.0)

    attention_field = attention_field * 0.7 + 0.3
    attention_field[~brain_mask] = 1.0  # No occlusion outside brain
    return image * attention_field, mask
